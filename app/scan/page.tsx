...
import { addMedicineToInventory } from '@/lib/inventory'
...
export default function ScanPage() {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const base64Image = image.split(',')[1]; // Remove data:image/jpeg;base64, prefix
      
      // Call the AI service to extract medicine information
      const result = await aiService.extractMedicineInfo(base64Image)
      setMedicineInfo(result)
  
      // Pre-fill the form with detected information
      setFormData({
        name: result.name,
        activeIngredient: result.dosage, // Using dosage as active ingredient
        dosage: result.dosage,
        quantity: result.quantity,
        expiryDate: result.expiryDate ? new Date(result.expiryDate) : undefined,
        frequency: "",
        notes: `Manufacturer: ${result.manufacturer}`,
      })
  ...
  const addToInventory = async () => {
    ...
    const { name, dosage, expiryDate, quantity, frequency, notes } = formData;

  
    // Step 2: Format expiry date (ensure it's YYYY-MM-DD)
  ...
  .insert([
    ...
    setFormData({
      name: "",
      activeIngredient: "", // optional UI field, not stored
      dosage: "",
      quantity: "",          // optional UI field, not stored
      expiryDate: undefined,
      frequency: "",         // optional UI field, not stored
      notes: "",             // optional UI field, not stored
    });
    setImage(null);
    setMedicineInfo(null);
    setShowConfirmation(false);
