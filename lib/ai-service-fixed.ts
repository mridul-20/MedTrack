...
Focus on common over-the-counter medications and general health advice. Always emphasize consulting healthcare professionals for serious concerns.`;
  ...
  async extractMedicineInfo(imageData: string): Promise<{
    name: string;
    dosage: string;
    expiryDate: string;
    manufacturer: string;
    ...
    try {
      const GEMINI_API_KEY = getGeminiApiKey();
      console.log("GEMINI_API_KEY in extractMedicineInfo:", GEMINI_API_KEY ? "Present" : "Missing");
      if (!GEMINI_API_KEY) {
        throw new Error("Gemini API key not configured");
      }

      const prompt = `Analyze this medicine package image and extract the following information in JSON format:
{
  "name": "Medicine name",
